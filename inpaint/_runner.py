...
class DiffusionConfig:
    ...
    @property
    def beta_schedule(self) -> list[float]:
...
class RunnerLightning(lightning.LightningModule):
    def __init__(
        ...
        super().__init__()
        self.config = config
        self.augment: Func = augment
        self.corrupt: Func = corrupt
        self.net: torch.nn.Module = net 
        self.diffusion = Diffusion(self.config.beta_schedule, self.device)
        # betas = make_beta_schedule(n_timestep=self.config.interval, linear_end=self.config.beta_max / self.config.interval)
        # betas = np.concatenate([betas[:self.config.interval//2], np.flip(betas[:self.config.interval//2])])
